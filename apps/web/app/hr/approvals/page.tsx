...
export default function HRApprovalsPage() {
  ...
  // Helper to deduct leave balance in eis_employee_leaves
  const deductLeaveDays = (empName: string, days: number) => {
    if (typeof window !== 'undefined') {
      const savedUsers = localStorage.getItem('eis_users');
      if (savedUsers) {
        try {
          const usersList: UserRow[] = JSON.parse(savedUsers);
          const found = usersList.find(u => u.name.toLowerCase() === empName.toLowerCase());
          if (found) {
            let leavesMap: Record<string, number> = {};
            const savedLeaves = localStorage.getItem('eis_employee_leaves');
            if (savedLeaves) {
              leavesMap = JSON.parse(savedLeaves);
            }
            // default is role-based leaves if not customized
            let initialLeaves = 14;
            if (found.role === 'ADMIN') initialLeaves = 24;
            else if (found.role === 'MANAGER') initialLeaves = 20;
            else if (found.role === 'HR') initialLeaves = 18;

            const currentBal = leavesMap[found.id] !== undefined ? leavesMap[found.id] : initialLeaves;
            leavesMap[found.id] = Math.max(0, currentBal - days);
            localStorage.setItem('eis_employee_leaves', JSON.stringify(leavesMap));
          }
        } catch (err) {
          console.error(err);
        }
  ...
  const handleApprove = (id: string) => {
    ...
    setRows((prev) => {
      const updated = prev.map((row) => (row.id === id ? { ...row, status: 'approved' as const } : row));
      const approvedRequest = prev.find(r => r.id === id);
      if (approvedRequest) {
        deductLeaveDays(approvedRequest.employee, approvedRequest.days);
      }
      return updated;
    });
    ...
    if (request) {
      ...
      triggerNotification(
        ...
        `Your leave request for ${request.days} days (${request.type}) has been fully approved by HR. Leave balance deducted.`,
        '/leave/history'
      );
    }
  };
  ...
  const handleReject = (id: string) => {
    const request = rows.find(r => r.id === id);
    setRows((prev) =>
      prev.map((row) => (row.id === id ? { ...row, status: 'rejected_hr' } : row))
    );
    ...
    if (request) {
      ...
      triggerNotification(
        ...
        `Your leave request for ${request.days} days (${request.type}) has been rejected by HR.`,
        '/leave/history'
      );
    }
  };
  ...
  const countPending = rows.filter(r => r.status === 'pending_hr').length;
      ...
      <main className="flex-1 p-8 text-slate-900 dark:text-slate-100">
        ...
        <div style={{ marginBottom: '24px' }}>
          ...
          <p style={{ color: "var(--text-muted, #64748b)", margin: '4px 0 0 0', fontSize: '14px' }}>
            {t('approvals', 'subtitle') || 'Process and track leave requests approved by department managers.'}
          </p>
        </div>
            ...
            <table className="approvals-table">
              ...
              <tbody>
                {filteredRows.map((row) => {
                  ...
                  const avatarClass = `user-avatar avatar-${details.role}`;
                    ...
                    <tr key={row.id}>
                      ...
                      <td>
                        ...
                        {row.status === 'pending_manager' && (
                          <span className="status-indicator indicator-approved" style={{ backgroundColor: '#fef3c7', color: '#d97706' }}>
                            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
                              <circle cx="12" cy="12" r="10" />
                              <polyline points="12 6 12 12 16 14" />
                            </svg>
                            {t('approvals', 'pendingManager') || 'Pending Manager'}
                          </span>
                        )}
                        ...
                        {row.status === 'rejected_manager' && (
                          <span className="status-indicator indicator-rejected">
                            ...
                            {t('approvals', 'rejectedByManager') || 'Rejected by Manager'}
                          </span>
                        )}
                      </td>
                    </tr>
